...
### Datasets
In the paper, two different synthetic datasets are used as source domain data separately. GTA5[1] consists of 24,966 annotated images with spatial resolution 1914×1052 originally which resized to 1280×720 and randomly cropped to the 1024×512 in training process.

CityScapes is used as target domain dataset. They use the 2,975 images from the training set as the target domain data for training. We test on the 500 validation images with dense
...
SYNTHIA→CityScapes.Lent(φw; Dt)
  
SYNTHIA[1] consists of 9400 annotated images with spatial resolution 1280×760, which are randomly cropped to 1024×512 before training process. The second dataset,  
